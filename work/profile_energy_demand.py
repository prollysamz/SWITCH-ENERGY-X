"""Check separate generation and demand coefficients with quadratic thermal loss.
...
def main():
    folder=ROOT/'runs/formula_demand_v3'
    ...
    drawdown=demand
    families={'gross_net':z['E_pg'],
        'delivered_net':z['E_pg']*(1-z['E_tl']),
        'usable_net':z['E_pm']}
    ...
    for name,net in families.items():
        ...
        for power in [2.]:
            ...
            for threshold in np.arange(15.,45.01,.25):
                ...
                fit=lsq_linear(A,target-3.9*qmean,bounds=([0,-10,-.02,-10],[3,0,.02,0]),tol=1e-10)
    ...
    report={'status':'Diagnostic profile only; no submission generated.',
        ...
        'limitations':['Generation and demand have separate coefficients; quadratic thermal loss and target mean 3.9 remain assumptions.',
