"""Profile compact energy formulas; reject inconsistent aggregate-score fits.
...
def main():
    folder=ROOT/'runs/formula_profile_v1'
    ...
    for name,net in families.items():
        ...
        for power in [1.,1.5,2.,2.5,3.]:
            best=None
            for threshold in np.arange(15.,45.01,.5):
                hm=H@np.maximum(centers-threshold,0)**power
                A=np.column_stack([fixed_moments,hm[:-1]-hm[-1]*qmean])
                fit=lsq_linear(A,target-3.9*qmean,bounds=([0,-3,-10],[3,0,0]),tol=1e-10)
                ...
                pred=3.9+fixed@fit.x[:2]+fit.x[2]*(heat-heat.mean())
    ...
    report={'status':'Diagnostic profile only; no submission generated.',
        ...
        'limitations':['The target mean 3.9 is assumed from approximate organizer statistics.',
