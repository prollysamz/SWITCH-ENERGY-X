"""Structural reconstruction with informed initialisation and physical bounds."""
import numpy as np
...
def map_latents(Y,p,sig=None,n_iter=22,need_cov=False):
    ...
    # Missing speed is still identifiable from its square or generation.
    missing_v=~ob[:,COL[4]]
    have_sq=missing_v & ob[:,COL[18]]
    th[have_sq,3]=np.sqrt(np.maximum(Y[have_sq,COL[18]],0))
    inferred_rho=th[:,4]*100/(287.05*(th[:,1]+273.15))
    wind_power=np.where(ob[:,COL[13]],Yf[:,COL[13]],
        np.where(ob[:,COL[14]],Yf[:,COL[14]]-0.43,
        (Yf[:,COL[21]]-50)/0.9+th[:,7]-0.43))
    have_power=missing_v & ~have_sq & (ob[:,COL[13]]|ob[:,COL[14]]|ob[:,COL[21]])
    vg=np.linspace(0,12.5,2001)
    wg=np.maximum.accumulate(p.wind(vg))
    th[have_power,3]=np.interp(wind_power[have_power]/inferred_rho[have_power],wg,vg)
    lo=np.array([0,-40,0,0,900,0,0.01,0])
    hi=np.array([1500,80,100,18,1100,1,1,2])
    th=np.clip(th,lo,hi)
    ...
    for it in range(n_iter):
        ...
        for _ in range(6):
            ...
            cand=np.clip(th+step,lo,hi); rc=resid_fn(cand,Yf,ob,W,p); cc=(rc*rc).sum(1)
            imp=(cc<cost)&~ok
